<script lang="ts">
  ...
  const powerUpNames: Record<string, { name: string; color: string }> = {
    ...
    boost: { name: gameMode === 'ground' ? 'JETPACK' : 'BOOST', color: '#44ffff' },
    ...
    if (stored) {
      try {
        ...
        customMaps = allCustomMaps.filter(map => {
          ...
          const requiredGame = gameMode === 'ground' ? 'blocky shooter' : 'starship flyer'
  ...
  async function loadDefaultMapThumbnail() {
    try {
      const mapFile = gameMode === 'ground' ? '/3d-maps/default_map.json' : '/3d-maps/Default_Space.json'
  ...
  function handleKeyDown(e: KeyboardEvent) {
    ...
    if (e.key === ' ' && boostCharges > 0 && !boostActive) {
      ...
      const msg = gameMode === 'ground' ? 'JETPACK ACTIVATED!' : 'BOOST ACTIVATED!'
  ...
  function updatePlayer(delta: number) {
    ...
    if (gameMode === 'ground') {
      ...
      const ground = scene.children.find(c => c instanceof THREE.Mesh && c.geometry instanceof THREE.PlaneGeometry && c.visible)
      if (ground && playerShip.position.y < 2) {
        ...
        if (health > 0 && now - lastDamageTime > 1000) {
          ...
          flash.className = 'absolute inset-0 bg-red-500/50 z-50 pointer-events-none'
  ...
  function updatePowerUps(delta: number) {
    ...
    powerUps = powerUps.filter(p => {
      ...
      if (playerShip!.position.distanceTo(p.mesh.position) < 6) {  // Larger collision radius for easier pickup
...
<div bind:this={container} class="relative flex-1 h-screen overflow-hidden">
  {#if showMapSelector}
    ...
    <div class="absolute inset-0 z-30 p-4 flex flex-col">
      ...
      <div class="flex justify-between items-center mb-4">
        <h1 class="text-4xl font-bold" style="color: #660460;">{gameMode === 'ground' ? '👽' : '🚀'} {gameMode === 'ground' ? 'Alien Attack' : 'Starship Flyer'}</h1>
            ...
            <div class="card-body p-4">
              ...
              <ul class="space-y-3 text-xs">
                ...
                <li class="flex items-center">
                  ...
                  {gameMode === 'ground' ? 'Jetpack - Fly for 15s' : 'Boost - Speed burst'}
                </li>
  ...
  {#if isPlaying && !isGameOver && !showLevelComplete && !isSpawning}
    ...
    {#if boostCharges > 0 && !boostActive}
      <div class="absolute bottom-4 left-1/2 transform -translate-x-1/2 text-white font-bold z-10 bg-black/70 p-3 rounded-lg border-2 border-cyan-500 animate-pulse">
        <div class="text-lg text-cyan-400 text-center">{gameMode === 'ground' ? 'JETPACK READY' : 'BOOST READY'}</div>
      </div>
    {:else if boostActive}
      <div class="absolute bottom-4 left-1/2 transform -translate-x-1/2 text-white font-bold z-10 bg-black/70 p-3 rounded-lg border-2 border-yellow-400">
        <div class="text-lg text-yellow-400 text-center animate-pulse">
          ...
          {#if gameMode === 'ground'}
            <span class="ml-2 text-white">({Math.ceil((boostEndTime - currentTime) / 1000)}s)</span>
          {/if}
        </div>
      </div>
    {/if}
    ...
    <div class="absolute bottom-4 right-4 text-white font-bold z-10 bg-black/70 p-4 rounded-lg border-2 border-yellow-500">
      ...
      <div class="text-xs text-gray-400 mt-2">↑↓ weapons | Space {gameMode === 'ground' ? 'jetpack' : 'boost'}</div>
    </div>
  {/if}
  ...
  {#if isLoading}
    <div class="absolute inset-0 bg-black z-50 flex flex-col items-center justify-center text-white">
      <div class="mb-4 text-2xl font-bold text-primary">{gameMode === 'ground' ? 'Alien Attack' : 'Starship Flyer'}</div>
      <div class="w-64 h-4 bg-gray-800 rounded-full overflow-hidden mb-2">
        <div class="h-full bg-accent transition-all duration-200" style="width: {loadingProgress}%"></div>
      </div>
      <div class="text-sm text-gray-400">{loadingStatus}</div>
