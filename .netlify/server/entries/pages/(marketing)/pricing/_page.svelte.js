import { a1 as head, $ as ensure_array_like, a0 as attr, Z as stringify } from "../../../../chunks/index2.js";
...
function _page($$renderer) {
  ...
  head("133tnjx", $$renderer, ($$renderer2) => {
    ...
    $$renderer2.push(`<meta name="description"${attr("content", `Pricing - ${stringify(WebsiteName)}`)}/>`);
  });
  $$renderer.push(`<div class="min-h-[70vh] pb-8 pt-[5vh] px-4"><h1 class="text-3xl font-bold text-center">Pricing</h1> <h2 class="text-xl text-center text-slate-500 mt-1 pb-3">Totally free, scale to millions of users</h2> <div class="w-full my-8">`);
  Pricing_module($$renderer, { callToAction: "Get Started", highlightedPlanId: "pro" });
  $$renderer.push(`<!----> <h1 class="text-2xl font-bold text-center mt-24">Pricing FAQ</h1> <div class="flex place-content-center"><div class="join join-vertical max-w-xl py-6 mx-auto"><div class="collapse collapse-arrow join-item border border-primary"><input type="radio" name="faq-accordion"/> <div class="collapse-title text-lg font-medium">Is this template free to use?</div> <div class="collapse-content"><p>Yup! This template is free to use for any project.</p></div></div> <div class="collapse collapse-arrow join-item border border-primary"><input type="radio" name="faq-accordion"/> <div class="collapse-title text-lg font-medium">Why does a free template have a pricing page?</div> <div class="collapse-content"><p>The pricing page is part of the boilerplate. It shows how the
  ...
  for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
    ...
    if (feature.header) {
      $$renderer.push("<!--[-->");
      $$renderer.push(`<tr class="bg-base-200 font-bold"><td colspan="3">${escape_html(feature.name)}</td></tr>`);
    } else {
      $$renderer.push("<!--[!-->");
      $$renderer.push(`<tr class="relative"><td>${escape_html(feature.name)}</td><td class="text-center">`);
      if (feature.freeString) {
        $$renderer.push("<!--[-->");
        $$renderer.push(`${escape_html(feature.freeString)}`);
      } else {
        $$renderer.push("<!--[!-->");
        if (feature.freeIncluded) {
          $$renderer.push("<!--[-->");
          $$renderer.push(`<svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 ml-2 inline text-success"><use href="#checkcircle"></use></svg>`);
        } else {
          $$renderer.push("<!--[!-->");
          $$renderer.push(`<svg xmlns="http://www.w3.org/2000/svg" class="w-[26px] h-[26px] inline text-base-200"><use href="#nocircle"></use></svg>`);
        }
        $$renderer.push(`<!--]-->`);
      }
      $$renderer.push(`<!--]--></td><td class="text-center">`);
      if (feature.proString) {
        $$renderer.push("<!--[-->");
        $$renderer.push(`${escape_html(feature.proString)}`);
      } else {
        $$renderer.push("<!--[!-->");
